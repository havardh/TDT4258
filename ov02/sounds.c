...
void smb_starman_theme ( void ) {
  ...
  set_track(0, variable_tune(pitch_high, 78, 0.875));
  set_track(1, variable_tune(pitch_middle, 78, 0.875));
  set_track(2, variable_tune(pitch_low, 57, 0.875));
}
...
void smb_1up( void ) {
	...
	set_track(0, even_tune(pitch, 6, EIGHT, 0.875));
}
...
void ex1( void ) {
  ...
  set_track(0, note_new(C, HALF, 0));
  set_track(1, note_new(A, HALF, 0));
